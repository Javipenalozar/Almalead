...
create type public.journal_dimension as enum ('observador', 'lenguaje', 'emocion', 'cuerpo');
...
create table public.module_schedule (
  id uuid primary key default gen_random_uuid(),
...
create table public.reflections (
  ...
  dimension public.journal_dimension not null,
  title text,
...
create table public.materials (
  ...
  title text not null,
  description text,
  section text not null default 'materiales',
...
create view public.student_journal_record
with (security_invoker = true)
as
select
...
create index idx_profiles_document_number on public.profiles (document_number);
create index idx_enrollments_student_id on public.enrollments (student_id);
create index idx_module_progress_enrollment_status on public.module_progress (enrollment_id, status);
create index idx_evaluation_submissions_student_status on public.evaluation_submissions (student_id, status);
create index idx_evidence_files_student_uploaded on public.evidence_files (student_id, uploaded_at desc);
create index idx_reflections_enrollment_created on public.reflections (enrollment_id, created_at desc);
create index idx_materials_section_module on public.materials (section, module_id);
create index idx_coach_messages_student_created on public.coach_messages (student_id, created_at desc);
...
create or replace function public.initialize_module_progress_for_enrollment()
returns trigger
language plpgsql
security definer
...
begin
  ...
  select
    ...
    m.id,
    case
      when ms.unlock_at is not null and ms.unlock_at <= now() then 'available'::public.progress_status
      else 'locked'::public.progress_status
    end
  from public.modules m
  left join public.module_schedule ms
    on ms.module_id = m.id
   and ms.cohort_id = new.cohort_id
...
create trigger initialize_module_progress_after_enrollment
after insert on public.enrollments
for each row execute function public.initialize_module_progress_for_enrollment();
...
alter table public.module_schedule enable row level security;
...
create policy "Signed in users can read module schedule"
on public.module_schedule for select
to authenticated
using (public.is_academic_staff() or exists (
  select 1
  from public.enrollments e
  where e.cohort_id = module_schedule.cohort_id
    and e.student_id = auth.uid()
));

create policy "Academic staff manage module schedule"
on public.module_schedule for all
using (public.is_academic_staff())
with check (public.is_academic_staff());
...
create policy "Academic staff manage material objects"
on storage.objects for all
to authenticated
using (bucket_id = 'almalead-materials' and public.is_academic_staff())
with check (bucket_id = 'almalead-materials' and public.is_academic_staff());

create policy "Students read material objects"
on storage.objects for select
to authenticated
using (
  bucket_id = 'almalead-materials'
  and (
    public.is_academic_staff()
    or exists (
      select 1
...
insert into public.cohorts (name, starts_on, schedule, modality, active)
values (
  'Certificación en Coaching Almalead 2026',
  '2026-04-21',
  'Martes 7:00 p. m. Colombia. Cada módulo se activa el miércoles siguiente a las 00:00 h.',
  'Virtual / presencial según cohorte',
  true
)
...
values
  ...
  (5, 'Escucha profunda y retroalimentación transformadora', 'Conversaciones poderosas, retroalimentación y cuidado del vínculo.', 'Registro de retroalimentación y escucha profunda en una práctica de rol.', 'Conversación', 1),
  ...
  (10, 'Práctica supervisada I', 'Casos guiados, prácticas de rol, grabaciones y retroalimentación supervisada.', 'Grabación de caso guiado con autoevaluación y retroalimentación.', 'Supervisión', 1),
  (11, 'Práctica supervisada II', 'Casos del participante, evidencias y retroalimentación con métricas de progreso.', 'Caso real del participante con evidencias y métricas.', 'Supervisión', 1),
...
insert into public.module_schedule (module_id, cohort_id, class_date, unlock_at)
select
  m.id,
  c.id,
  schedule.class_date::date,
  schedule.unlock_at::timestamptz
from (
  values
    (1, '2026-04-28', '2026-04-29T00:00:00-05:00'),
    (2, '2026-05-05', '2026-05-06T00:00:00-05:00'),
    (3, '2026-05-12', '2026-05-13T00:00:00-05:00'),
    (4, '2026-05-19', '2026-05-20T00:00:00-05:00'),
    (5, '2026-05-26', '2026-05-27T00:00:00-05:00'),
    (6, '2026-06-02', '2026-06-03T00:00:00-05:00'),
    (7, '2026-06-09', '2026-06-10T00:00:00-05:00'),
    (8, '2026-06-16', '2026-06-17T00:00:00-05:00'),
    (9, '2026-06-23', '2026-06-24T00:00:00-05:00'),
    (10, '2026-06-30', '2026-07-01T00:00:00-05:00'),
    (11, '2026-07-07', '2026-07-08T00:00:00-05:00'),
    (12, '2026-07-14', '2026-07-15T00:00:00-05:00')
) as schedule(module_number, class_date, unlock_at)
join public.modules m on m.module_number = schedule.module_number
join public.cohorts c on c.name = 'Certificación en Coaching Almalead 2026'
on conflict (module_id, cohort_id) do update
set
...
values
  ...
  ('Práctica supervisada', 'Prácticas de rol, grabaciones, retroalimentación y casos reales de la cohorte.', 'worksheet', null),
